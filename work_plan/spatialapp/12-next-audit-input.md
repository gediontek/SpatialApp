...
- ✅ **N43 (High) — WebSocket bearer token in query string.** `blueprints/websocket.py:73` read the bearer from `?token=...`, which appears in proxy / nginx / middleware access logs in plaintext (Authorization headers used by REST are typically redacted; query params aren't). **Fix**: `handle_connect(auth=None)` already had the Socket.IO handshake `auth` dict plumbed in but ignored — now reads `auth.token` first (out-of-URL), falls back to `?token=` for backward compat. JS client `static/js/chat.js:59` switched from `query: { token }` to `auth: { token }` so new UI connections never put the token in a URL. **3 regression tests** in `tests/test_websocket.py::TestWebSocketConnect::test_n43_*`: auth dict accepted, query string still accepted (compat), auth dict takes precedence over query (catches future regressions where someone re-orders the lookups).

- ⏭ **N44 (High, deferred to Cycle 28) — Token revocation doesn't disconnect WS.** `_sid_user_map` caches user identity at connect time. When a user's token is revoked in the DB, their existing WS connection keeps operating under the cached identity until they voluntarily disconnect. Exploitation requires DB-write access (operator-driven), so the threat surface is narrow, but the gap is real. **Scoped fix for Cycle 28**: helper `revoke_user_websocket_sessions(user_id)` that finds all SIDs in `_sid_user_map` for that user and force-disconnects them, plus an admin endpoint or hook that operators call when revoking. Not closed this cycle to keep Cycle 27 focused on the cleaner N43 fix.

**Verification**: `make eval` green; full unit suite **1,606 passed / 11 skipped / 0 failed** (+3 from N43 tests, ~103s, zero regressions).

**Why this matters**: N43 closes a passive token-leak vector that was active on every WebSocket handshake in production-like deploys behind nginx (the most common architecture). N44 closure (next cycle) will close the active token-revocation gap. Together they harden the WS auth surface to the same level as the REST auth surface (which is already at full coverage).
